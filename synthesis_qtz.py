...
def saveaudio(cfg, wave, tp, ns):
    ...
    wav_name = 'samples/{}/{}_{}_{}_{}.wav'.format(cfg['model_label_s'], cfg['note'], cfg['epoch_s'], tp, str(ns))
...
def synthesis(cfg):
    ...
    # load test data
    if cfg['orig']:
        test_dataset = Libri_lpc_data_orig('val', tot_chunks, qtz=0) 
    else:
        test_dataset = Libri_lpc_data('val', tot_chunks, qtz=0)
    ...
    for ns, (sample_name, sample, c, nm_c) in enumerate(test_loader):
        ...
        c_in, r, r_qtz = model_f.encoder(feat=feat, n_dim=cfg['code_dim'])
        # mask 
        e, lpc_c, rc = ceps2lpc_v((c_in*MAXI).reshape(-1, c_in.shape[-1]).cpu()) # lpc_c - (N*(L-1), 16)
        # lpc = c[:, 2:-2, -16:].to(torch.float).to(device) # (1, tot*15, 16)  # Use 
        all_features = torch.cat((c_in.cpu(), lpc_c.unsqueeze(0)), -1)
        # all_features = torch.cat((c_in.cpu(), lpc.cpu()), -1)
        print(all_features.shape)
        np.save('../LPCNet/generated_features/{}_newlpc.npy'.format(sample_name[0]), \
